...
1. If `TIKBOT_VERSION` is set, TikBot uses that value.
2. If the current commit is tagged like `v1.2.3`, TikBot uses `1.2.3`.
3. Otherwise, TikBot derives a version from git history as `1.<commit-count>.0`.
